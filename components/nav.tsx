...
const Navbar = () => {
  ...
  return (
    <div className={`z-[1000] w-full duration-500 fixed h-[10vh] backdrop-blur-navBar ${scrollingDown ? "-translate-y-16" : "translate-y-0"}`}>
      <div className="w-full h-full items-center justify-between flex text-black px-14 mt-0 py-5"
      >
      ...
      <div className="flex">
        {["Services", "Our Work", "About us", "Insights", "Contact us"].map(
          (val, ind) => {
            return <a key={ind} className={`px-4 ${ind === 4 && "ml-44 px-0"} text-[17px] tracking-tight cursor-pointer`}>{val}</a>;
          }
        )}
      </div>
    </div>
