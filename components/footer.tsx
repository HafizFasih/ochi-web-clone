...
const Footer = () => {
  return (
    <div className="relative w-full pt-10 flex bg-zinc-100">
      <div className="px-16 py-2 w-2/4">
        <h1 className='text-[9vw] font-semibold leading-[7vw] uppercase text-zinc-900 font-["FoundersGrotesk"]'>
          eye- <br />
          opening
        </h1>
        <div className="logo absolute bottom-10 left-16">
          <svg
            width="32"
            height="15"
            viewBox="0 0 72 30"
      ...
      <div className="w-2/4 ">
        ...
        <div className="py-8 text-sm inline-block">
            {["S:", "Instagram", "Behance", "Facebook", "Linkedin", "L:","202-1965 W 4th Ave","Vancouver, Canada", "30 Chukarina St", "Lviv, Ukraine","E:", "hello@ochi.design"].map((val,ind)=> <h3 key={ind} className={`text-black  ${val.length === 2 ? "py-4": "underline"} `} style={{textDecorationThickness: "0.5px"}}>{val}</h3>)}
            <h3 className="text-zinc-400 mt-10">&copy; ochi design 2024. <u>Legal Terms</u></h3>
        </div>
        <div className="text-black px-16 flex flex-col justify-center text-sm relative">
            {["M:","Home","Services","Our Work","About us", "Insights", "Contact us"].map((val,ind)=> <h3 key={ind} className={`${val === "M:" ? "py-4" : "underline"}`}
            style={{textDecorationThickness: "0.5px"}}
            >{val}</h3>)}
            <h3 className="absolute bottom-8 right-6 text-zinc-400">Website by Obys</h3>
        </div>
      </div>
      </div>
    </div>
